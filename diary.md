...
*   **`internal/adapters/nats/publish.go` (TM Task 7: Implement NATS JetStream Publisher):**
    ...
    *   `NewJetStreamPublisher` now accepts an existing `*nats.Conn`, gets a JetStream context, and ensures the target `wa_stream` (configured via `config.KeyJSWebsocketStreamName` and `config.KeyJSWebsocketStreamSubjects`) exists, creating it if necessary.
...
*   **`internal/adapters/config/viper.go` (supporting TM Task 7):**
    *   Added the constant `KeyJSWebsocketStreamSubjects` and set its default value to `"websocket.>"` in `NewViperConfigProvider` to support the NATS publisher configuration. This resolves the remaining linter error in `publish.go`.
...
*   Corrected the `expectedPublishedSubject` in `integration_test/main_integration_suite_test.go` to match the actual subject generated by `transform_service.go` for messages (`websocket.{companyID}.{agentID}.messages.{chatID}`).
