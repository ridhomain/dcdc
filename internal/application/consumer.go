...
// extractTableName parses the NATS subject (e.g., cdc.company_id.table_name)
// and returns the table name component.
// It expects the subject to have at least 3 parts separated by dots.
func extractTableName(subject string) (string, error) {
	parts := strings.Split(subject, ".")
	if len(parts) < 3 {
		return "", fmt.Errorf("invalid NATS subject format: expected at least 3 parts, got %d for subject '%s'", len(parts), subject)
	}
	// As per PRD §1 and F-1, subject is cdc.<company>.<table>
	// So, parts[0] is "cdc", parts[1] is <company>, parts[2] is <table>.
	tableName := parts[2]
	if tableName == "" {
		return "", fmt.Errorf("extracted table name is empty from subject '%s'", subject)
	}
...
func (c *Consumer) processEvent(ctx context.Context, msg CDCEventMessage, originalSubject string) {
	...
	var err error
	if tn, ok := ctx.Value(logger.LogKeyTable).(string); ok && tn != "" {
		tableName = tn
	} else {
		tableName, err = extractTableName(originalSubject)
		if err != nil {
			c.logger.Error(ctx, "Worker: Failed to re-extract table name", zap.Error(err), zap.String("subject", originalSubject))
	...
	enrichedPayload, targetSubject, payloadBytes, transformErr := c.eventTransformer.TransformAndEnrich(ctx, &cdcEventData, originalSubject, tableName)

	if transformErr != nil {
		c.logger.Error(ctx, "Event transformation failed", zap.Error(transformErr), zap.String("table_name", tableName))
		...
		if errors.As(transformErr, &dataProcessingErr) || // If it's a wrapped data processing error
			...
			c.metricsSink.IncEventsTotal(tableName, "transform_data_error") // General metric for consumer
			_ = msg.Ack()                                                   // Data quality/validation errors from transform are non-retryable here.
		} else {
			c.metricsSink.IncEventsTotal(tableName, "transform_system_error") // Unexpected error in transform
	...
	if err != nil {
		...
		c.metricsSink.IncEventsTotal(tableName, "dedup_error")
		c.incrementConsecutiveFailures(ctx) // Panic Guard: Increment
	...
	if isDup {
		...
		c.metricsSink.IncEventsTotal(tableName, "duplicate")
	...
	if publishErr != nil {
		...
		c.metricsSink.IncEventsTotal(tableName, "publish_error")
		c.incrementConsecutiveFailures(ctx) // Panic Guard: Increment
	...
	c.metricsSink.ObserveProcessingDuration(tableName, processingDuration)
	c.metricsSink.IncEventsTotal(tableName, "processed")
...
func (c *Consumer) HandleCDCEvent(ctx context.Context, msg CDCEventMessage) error {
	...
	tableName, err := extractTableName(originalSubject)
	if err != nil {
		c.logger.Error(ctx, "Failed to extract table name from subject", zap.Error(err), zap.String("subject", originalSubject))
		...
		return err // Return error, but message is ACKed.
	}
	ctx = context.WithValue(ctx, logger.LogKeyTable, tableName)
	...
	if _, isAllowed := domain.AllowedTables[tableName]; !isAllowed {
		c.logger.Info(ctx, "Table not allowed, skipping event before worker submission")
		c.metricsSink.IncEventsTotal(tableName, "skipped")
		if ackErr := msg.Ack(); ackErr != nil {
			c.logger.Error(ctx, "Failed to ACK skipped message for disallowed table", zap.Error(ackErr))
		}
	...
	if submitErr != nil {
		...
		c.metricsSink.IncEventsTotal(tableName, "worker_submit_error")
